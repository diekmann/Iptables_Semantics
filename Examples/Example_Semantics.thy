...
text{*we use a primitive matcher which always apllies.*}
  ...
  "applies_exampleMatchExact (Src (Ip4Addr addr)) p \<longleftrightarrow> src_ip p = (ipv4addr_of_dotdecimal addr)" |
  "applies_exampleMatchExact (Dst (Ip4Addr addr)) p \<longleftrightarrow> dst_ip p = (ipv4addr_of_dotdecimal addr)" |
  ...
  lemma "[''FORWARD'' \<mapsto> [ Rule (MatchAnd (Match (Src (Ip4Addr (0,0,0,0)))) (Match (Dst (Ip4Addr (0,0,0,0))))) Reject, 
         ...
         ],applies_exampleMatchExact,\<lparr>src_ip=(ipv4addr_of_dotdecimal (1,2,3,4)), dst_ip=(ipv4addr_of_dotdecimal (0,0,0,0)), prot=protPacket.ProtTCP\<rparr>\<turnstile>
            \<langle>[Rule MatchAny (Call ''FORWARD'')], Undecided\<rangle> \<Rightarrow> (Decision FinalAllow)"
  apply(rule call_result)
  apply(auto)
  apply(rule Semantics.seq_cons)
