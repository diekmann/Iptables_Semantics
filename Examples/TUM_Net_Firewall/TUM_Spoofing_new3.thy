...
subsection{*Printing various version of the simplified firewall*}
  ...
  text{*Abstracting over the interfaces removesa lot of information*}
  value[code] "let x = to_simple_firewall (upper_closure
  ...
  value[code] "let x = to_simple_firewall (lower_closure
                      ...
                      (optimize_matches (iiface_constrain (map_of_ipassmt ipassmt))
                      (ctstate_assume_new
                      (lower_closure (unfold_ruleset_FORWARD net_fw_1_FORWARD_default_policy (map_of net_fw_1))))))))
...
subsection{*Checking spoofing Protection*}
