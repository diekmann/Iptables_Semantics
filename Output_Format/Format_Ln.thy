...
text{* assumes: @{const "normalized_nnf_match"}*}
...
*}
lemma iptrule_match_collect_by_primitive_extractor: "normalized_nnf_match m \<Longrightarrow> iptrule_match_collect m (UncompressedFormattedMatch [] [] [] []) = (
...
lemma UncompressedFormattedMatch_to_match_expr_correct: assumes "normalized_nnf_match m" and "matches \<gamma> (UncompressedFormattedMatch_to_match_expr accu) a p" shows
...
using assms apply (induction m accu arbitrary: rule: iptrule_match_collect.induct)
  ...
  apply(simp_all) --{*@{text "\<not> normalized_nnf_match"}*}
done
...
corollary format_Ln_match_correct: "normalized_nnf_match m \<Longrightarrow> matches \<gamma> (UncompressedFormattedMatch_to_match_expr (format_Ln_match m)) a p \<longleftrightarrow> matches \<gamma> m a p"
...
corollary "normalized_nnf_match m \<Longrightarrow> matches \<gamma> (UncompressedFormattedMatch_to_match_expr (format_Ln_match m)) a p \<longleftrightarrow> matches \<gamma> m a p"
proof -
  {fix yc
    have "normalized_nnf_match yc \<Longrightarrow> \<not> has_disc is_Dst yc \<Longrightarrow> \<not> has_disc is_Prot yc \<Longrightarrow> \<not> has_disc is_Extra yc \<Longrightarrow> \<not> has_disc is_Src yc \<Longrightarrow> matches \<gamma> yc a p"
    apply(induction yc)
  ...
  assume normalized: "normalized_nnf_match m" 
...
lemma format_Ln_match_correct': "\<forall> m' \<in> set ms. normalized_nnf_match m' \<Longrightarrow> 
...
definition format_Ln_rules_uncompressed :: "iptrule_match rule list \<Rightarrow> (match_Ln_uncompressed \<times> action) list" where
  "format_Ln_rules_uncompressed rs = [((format_Ln_match (get_match r)), (get_action r)). r \<leftarrow> (normalize_rules_dnf rs)]"
...
case (Cons r rs)
  ...
  let ?rsA="map ((\<lambda>(m, y). Rule (UncompressedFormattedMatch_to_match_expr m) y) \<circ> (\<lambda>r. (format_Ln_match (get_match r), get_action r))) (normalize_rules_dnf [r])"
  let ?rsC="map ((\<lambda>(m, y). Rule (UncompressedFormattedMatch_to_match_expr m) y) \<circ> (\<lambda>r. (format_Ln_match (get_match r), get_action r))) (normalize_rules_dnf rs)"
  ...
  have "approximating_bigstep_fun \<gamma> p (Ln_rules_to_rule (format_Ln_rules_uncompressed (r # rs))) s = 
        approximating_bigstep_fun \<gamma> p
             (map ((\<lambda>(m, y). Rule (UncompressedFormattedMatch_to_match_expr m) y) \<circ> (\<lambda>r. (format_Ln_match (get_match r), get_action r))) (normalize_rules_dnf (r # rs))) s"
    by (simp add: Ln_rules_to_rule_def format_Ln_rules_uncompressed_def)
  also have "... = approximating_bigstep_fun \<gamma> p (?rsA @ ?rsC) s" using normalize_rules_fst normalize_rules_dnf_def2 by (metis map_append) (*this is so broken \<dots>*)
    ...
    next
      show "approximating_bigstep_fun \<gamma> p ?rsA s = approximating_bigstep_fun \<gamma> p [r] s"
       ...
       apply(subst format_Ln_match_correct'[symmetric])
        apply(simp add: normalized_nnf_match_normalize_match)
       apply(simp)
       done
     next
