...
subsection\<open>Sets of IP addresses\<close>
  ...
  text\<open>Example (pseudo syntax):
    @{const ipset_from_netmask}@{text "192.168.1.129  255.255.255.0 = {192.168.1.0 .. 192.168.1.255}"}\<close>
  ...
  text\<open>Specifying sets in CIDR notation: 192.168.0.0/24\<close>
  ...
  text\<open>A prefix lengths of word size gives back the singleton set with the IP address.
  ...
  lemma ipset_from_cidr_alt: 
    ...
    shows "ipset_from_cidr pre len =
            {(pre AND ((mask len) << (len_of(TYPE('i)) - len))) .. pre OR (mask (len_of(TYPE('i)) - len))}"
  ...
  text\<open>Another definition of CIDR notation:
       All IP addresse which are equal on the first @{text "len - n"} bits\<close>
...
subsection\<open>Clever Operations on IP Addresses in CIDR Notation\<close>
  ...
  text{*Intersecting with an address with prefix length zero always yields non-empty result.*}
