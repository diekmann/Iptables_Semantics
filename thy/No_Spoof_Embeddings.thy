 ...
 (*expressed as set*)
  ...
  corollary no_spoofing_executable_set_preprocessed:
      ...
      assumes "matcher_agree_on_exact_matches \<gamma> common_matcher"
          ...
          and "iface \<in> dom ipassmt"
      shows "{p_src p | p . newpkt p \<and> \<Gamma>,\<gamma>,p\<lparr>p_iiface:=iface_sel iface\<rparr>\<turnstile> \<langle>rs, Undecided\<rangle> \<Rightarrow> Decision FinalAllow} \<subseteq>
                 ipv4cidr_union_set (set (the (ipassmt iface)))"
  proof -
