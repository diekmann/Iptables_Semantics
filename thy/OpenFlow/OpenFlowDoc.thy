...
However, we perceived the \emph{a priori} lookup of the MAC addresses as rather inelegant and built a second model.\<close>
...
This can only be achieved by changing the subnet of all connected device, moving them into one common subnet.
\<close>
text\<open>
...
text\<open>Table 3 of Section 3.1 of \cite{specification10} gives a list of require packet fields that can be used to match against.
...
Nevertheless, the OpenFlow reference implementation \cite{openvswitch} supports them.
We will explain in detail why we chose to include bitmasks on layer 4 ports in Section~\ref{sec:convi}.\<close>
...
This mirrors the faulty behavior of the aforementioned implementation. 
When adopting it into our model, it means that any packet matches against the field match set @{term "{IPv4Dst (PrefixMatch 134744072 32)}"} 
...
Guha \emph{et al.}~\cite{guha2013machine} have dealt with overlapping, as explained in Section~\ref{sec:sdn}.
However, the semantics for a flow table they presented in their paper and that is shown in Figure~\ref{fig:guhafts} 
...
We have tried to reproduce the original inductive definition (while keeping our abstraction @{term \<gamma>}),
 in Isabelle/HOL\footnote{The original is written in Coq~\cite{barras1997coq} and we can not directly use it.}:\<close>
...
text\<open>The main difficulties for @{const simple_match_to_of_match} lie in making sure that the prerequisites are satisfied
 and in the fact that a @{type simple_match} has slightly stronger match expression.
\begin{itemize}
...
This is, in theory, possible if we give the firewall rules representing the routing table a match on the output port, additionally to the match on the destination address.
This will have two effects: Matches on the output interface from @{term fw} will only appear in conjunction with the match on the destination IP address that the routing rule dictates.
Also, packets that have the ``correct'' (as decided by @{term "routing_table_semantics rt"} output port set) will still match the same rule, but packets with an incorrect port will not match any of the preceding rules.
This property can be lifted through the join and then it can be shown that what holds for a packet with the correct output interface holds for a packet with any (i.e. an unset) output interface and the output interface matches set to @{const ifaceAny}.
However, showing that this is correct is highly technical.
...
The first 6 assumptions are validity criteria we require on the input data.
Instead of justifying them theoretically, we just leave it to the evaluation in Section~\ref{sec:eval} to test these criteria on a few real data sets.
The next two assumptions are limitations on the traffic we make a statement about.
...
The last assumption is that the translation does not return a runtime error.\<close>

subsubsection\<open>Comparison to Exodus\<close>
text\<open>
  We are not the first researchers to attempt automated migration to SDN.
...
\begin{itemize}
  ...
  \item Nelson \emph{et al.} reject the idea that the translation should be proven correct.
\end{itemize}
\<close>
