...
lemma match_iface_simple_match_any_simps:
  ...
  apply(simp_all add: simple_match_any_def match_ifaceAny ipv4range_set_from_prefix_0)
...
theorem transform_simple_fw_upper:
  ...
  { fix m a
    ...
    have 1: "{p. ?\<gamma>,p\<turnstile> \<langle>?rs', Undecided\<rangle> \<Rightarrow>\<^sub>\<alpha> Decision FinalAllow \<and> newpkt p} =
      ...
      by simp
    from abstract_primitive_in_doubt_allow(2)[OF matcher_generic_common_matcher nnf2 s2] have 2:
